...
function row_col_equilibration(A, tol::AbstractFloat)
    ...
    while !( (max_r <= tol) && (max_s <= tol) )
        ...
        for i = 1:n
            ...
            r[i] = inv( sqrt( norm(@view(tmpA[i, :]), Inf) ) )
            s[i] = inv( sqrt( norm(@view(tmpA[:, i]), Inf) ) )
        ...
        max_r = maximum(abs.(r .- 1))
        max_s = maximum(abs.(s .- 1))
...
"""
Rounds given matrix A to half precision, scaling all elements to avoid overflow.
"""
